...
get_nmcli() {
    ...
    if [ -n "$devinfo" ]; then
        dev=${devinfo%%|*}
        type=${devinfo##*|}
        # ip=$(nmcli -g IP4.ADDRESS device show "$dev" 2>/dev/null | head -n1 | cut -d'/' -f1) # IP variable definition kept for 'result' structure, but not used in final output logic
        ip="" # Set IP to empty string
        ssid=""
        if [ "$type" = "wifi" ]; then
...
get_iw() {
    ...
    for w in $(iw dev 2>/dev/null | awk '/Interface/{print $2}'); do
        if iw dev "$w" link 2>/dev/null | grep -q 'Connected'; then
            # ip=$(ip -4 -o addr show dev "$w" 2>/dev/null | awk '{print $4}' | cut -d'/' -f1 | head -n1) # IP variable definition kept for 'result' structure, but not used in final output logic
            ip="" # Set IP to empty string
...
get_route() {
    ...
    if [ -n "$dev" ]; then
        ...
        # echo "$dev|$type|$src||$conn" # Original line
        echo "$dev|$type|||$conn" # Removing src (IP) from output structure
        return 0
    fi
    return 1
}
...
# Build output: only show SSID for wifi, and just the icon for wired
if [ "$type" = "wifi" ]; then
    if [ -n "$ssid" ]; then
        # Original: printf "%s %s %s%s" "$icon" "$ssid" "$ipaddr" "$indicator"
        # Original: printf "%s %s%s" "$icon" "$ssid" "$indicator"
        printf "%s %s%s" "$icon" "$ssid" "$indicator"
    else
        # Original: printf "%s %s%s" "$icon" "$ipaddr" "$indicator"
        # Original: printf "%s%s" "$icon" "$indicator"
